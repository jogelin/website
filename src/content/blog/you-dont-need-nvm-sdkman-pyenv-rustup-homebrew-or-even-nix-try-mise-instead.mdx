---
title: "You Don’t Need NVM, SDKMAN, Pyenv, Rustup, Homebrew or even Nix. Try Mise Instead."
subtitle: "Mise can manage all your development environments across multiple technologies"
publishedAt: 2025-11-24
coverImage: /blog/covers/you-dont-need-nvm-sdkman-pyenv-rustup-homebrew-or-even-nix-try-mise-instead.jpeg
tags:
  - nx
  - monorepo
  - package-management
...
Most of the time, this means integrating Nx — a polyglot toolchain and ecosystem that unifies how we build, test, lint, release, and cache projects, no matter the stack.

![](\/blog/images/0*GOVJNQcMZMErdUBW.png)
...
![](\/blog/images/1*JZu2qsqQLnoq5HSFZKKIJg.jpeg)
...
**After that, it’s magic.** Just navigate into a folder, and the right versions are instantly available — no manual commands required.
...
It’s very handy: even if your OS has other versions installed, Mise always picks the closest configuration (local folder first, then global).
...
Mise can also handle environment variables — great for local secrets.
...
![](\/blog/images/1*Q_mpSuZbVUK1Tm-rdxinfQ.jpeg)
...
* One giant image with everything → slow, heavy, hard to maintain
    
* Multiple images per stack → complex matrix management
    
* Installing tools manually in pipelines → messy scripts, hard to maintain
    

Your CI only needs **one thing**: **Mise**

Then run `mise install` and everything defined in `mise.toml` will be installed—fast and clean.

**Caching** is also easier because all binaries land in a predictable location.

For GitHub Actions, there’s even a ready-made action: [`jdx/mise-action@v3`](https://github.com/jdx/mise-action)
...
If your company uses Nix, you might already know tools like `devbox`, `devshell`, `devenv`, `flox`…

And maybe you also struggled with some of them.

There’s a great plugin by **José Badeau** that makes Nix as easy as writing your Mise config:
...
If you’re using Nx Cloud with Nx Agents, there’s even a [workflow step to install Mise automatically](https://github.com/nrwl/nx-cloud-workflows/tree/main/workflow-steps/install-mise):
...
Overall, Mise keeps your projects consistent, your machine clean, and your workflows easier to maintain — whether you’re working locally or in CI.

## **Credits**

Big thanks to **José Badeau** and **Miguel Sanchez** who introduced me to Mise.

<WantToGoFurther description="If you're curious about using Mise with Nx or integrating it into complex CI/CD pipelines, feel free to reach out — I'm always happy to share real-world setups and practical tips:" />