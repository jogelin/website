---
title: "🩹 Nx Crystal Plugin Picking the Essentials"
subtitle: "Exploring Workarounds and Custom Solutions for Early Days of Nx 18"
publishedAt: 2024-03-23
coverImage: /blog/covers/nx-crystal-plugin-picking-the-essentials.png
tags:
  - nx
  - monorepo
  - typescript
...
As specified in my article, [⛔ Target Exclusions in Nx Project Crystal](https://medium.com/@jgelin/target-exclusions-in-nx-project-crystal-fbabce816b21), there are use cases that necessitate extending the Nx Project Crystal. However, it is not possible to hook only a specific part of the plugin. You either take it all or you don’t use it at all. Only the `createNode` function is exposed, but that function does more than one thing.
...
You can check the section [Create Your Crystal Plugin](/blog/discovering-nx-project-crystals-magic#heading-create-your-crystal-plugin) of my article [💎 Discovering Nx Project Crystal's Magic](/blog/discovering-nx-project-crystals-magic) if you want to know what I am talking about 🙃

So, if I want to use the `@nx/jest/plugin` for the generation of the `test` target, I’ll need to expose the internal function [`buildJestTargets`](https://github.com/nrwl/nx/blob/286e83dbe9233bd4fb227658a27bc79796694b85/packages/jest/src/plugins/plugin.ts#L116). To do so, I'll need to patch the `@nx/jest/plugin` library 😈
