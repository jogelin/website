...
coverImage: https://cdn.hashnode.com/res/hashnode/image/upload/v1718740608019/a69281f6-379a-449f-a98e-fc6f3180c1e0.png
canonicalUrl: https://gelinjo.hashnode.dev/deep-dive-into-nx-affected
tags:
  - nx
  - monorepo
  - angularjs
...
When working on a **large code base** in a Monorepo, you’ll have a repository that contains multiple Applications and Libraries.
...
![Affected Projects Principle](https://cdn-images-1.medium.com/max/800/1*wBNqBAJJrM2AemYdHiIjUg.png)
...
![Comparaison between Project Graph and Task Graph](https://cdn-images-1.medium.com/max/800/1*oiv9ahvmBu6yPWUzcPr_0g.png)
...
![Nx Affected Command Diagram](https://cdn-images-1.medium.com/max/800/1*a4NepMkOaE4qTKiyiVKz5g.png)

With this command, you’ll be able to run only the list of tasks that are affected.
...
![Nx Show Command Diagram](https://cdn-images-1.medium.com/max/800/1*IjfK5YGsNCkJlWq89IbqkA.png)
...
![Nx Graph Command Diagram](https://cdn-images-1.medium.com/max/800/1*2FEVl4rju5nX9bl_X_LBZQ.png)

It will open a web page where you can see a graph similar to this:

![Affected Graph Visualization](https://cdn-images-1.medium.com/max/800/1*OJy1WanSXdIFktjIzlEJmA.png)
...
![Nx Affected Process Overview](https://cdn-images-1.medium.com/max/800/1*x1WIkF9FfQyJKwSJ5pANrg.png)

### Step 1 - Find Touched Files

Before computing the list of affected projects, Nx loads the list of modified/touched files:

![Find Touched Files](https://cdn-images-1.medium.com/max/800/1*TiujNtR8IAEEijt-Sl2Nrg.png)
...
![Find Affected Nodes from Paths](https://cdn.hashnode.com/res/hashnode/image/upload/v1719221936229/fd10a4ca-d6bd-4c41-8f13-ea20936aae17.png)
...
![](https://cdn.hashnode.com/res/hashnode/image/upload/v1719666582167/7b68d443-29a9-47fd-80cf-7dec1b8fd1b7.png)
...
![Find Affected Nodes from Plugins](https://cdn-images-1.medium.com/max/800/1*sH9pzg8FUlt2KbS195Lw7g.png)
...
![Find Affected Nodes from Npm Dependencies](https://cdn-images-1.medium.com/max/800/1*kY_JSaI0-Yqo8jrLNAZC8A.png)

By default, modifying the **package manager lock** file affects **all projects:**

![Find Affected Nodes from Lock Files](https://cdn-images-1.medium.com/max/800/1*utuEVaT7yZxyq1g6iGW3Tw.png)
...
![Find Affected Nodes From Typescript Configuration](https://cdn.hashnode.com/res/hashnode/image/upload/v1719221995174/a2b00cc8-2916-4be4-853a-0d6322fb5be8.png)
...
![Find Affected Nodes From Global Files](https://cdn-images-1.medium.com/max/800/1*GYZDCbdVMZKBisG1qnh5TA.png)
...
![Generate Affected Graph](https://cdn-images-1.medium.com/max/800/1*cM_iT8c8onYutt3ilK4HcQ.png)
...
![Generate Affected Graph For Deps](https://cdn-images-1.medium.com/max/800/1*3tgA1mZsxFUxzxC-ZFgDmw.png)
...
If you still don’t understand why some projects are affected on a branch, you can always debug the Nx affected process.

### By using Nx Graph

If you open the Nx graph with the affected command, you’ll be able to see all affected projects as specified in the [🤓 Affected Reminder](#9d37) section.
...
Currently, there is limited customization for the affected process. You can customize it when updating a dependency by using the configuration `projectsAffectedByDependencyUpdates` (see Step 2.4 — Find Affected Nodes from Npm Dependencies).

### Patch Nx

> ⚠️ Only use this as a last resort!

This is a **hacky** solution, but I use it to **customize** the affected process. **Patching** your **Nx** library using your package manager’s patch system allows you to **change** the **rules**.

For example, if fixes take time to implement, you can **disable** the “**Affected All”** use cases with:
