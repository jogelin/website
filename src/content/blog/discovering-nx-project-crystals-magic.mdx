...
coverImage: https://cdn.hashnode.com/res/hashnode/image/upload/v1708965023428/d43cb50e-af43-4b73-b37c-d4ba90ec0c61.webp
canonicalUrl: https://gelinjo.hashnode.dev/discovering-nx-project-crystals-magic
tags:
  - nx
  - angularjs
  - reactjs
...
* [✌️ Nx Plugin v2: Dynamic Project Configurations](https://jgelin.medium.com/%EF%B8%8F-nx-plugin-v2-dynamic-project-configurations-6055ba700105)
    

I’m excited to discuss again one of my favorite Nx features: **“The Inferred Project Configurations.”** The [Nx](https://medium.com/u/2817fb68583?source=post_page-----7f42faf2a135--------------------------------) team now introduces a unified approach known as [**Nx Project Crystal**](https://nx.dev/concepts/inferred-tasks) 💎.
...
To grasp the concept of [**Nx Project Crystal**](https://nx.dev/concepts/inferred-tasks) 💎, let’s revisit the [Nx](https://medium.com/u/2817fb68583?source=post_page-----7f42faf2a135--------------------------------) team’s journey in project configuration.

## **1\. angular.json**

***“In the beginning, there was nothing”.*** The [Nx](https://medium.com/u/2817fb68583?source=post_page-----7f42faf2a135--------------------------------) team began defining a monorepo structure for apps and libs using [`angular.json`](https://angular.io/guide/workspace-config), tied to [Angular CLI](https://angular.io/cli):

![](https://miro.medium.com/v2/resize:fit:700/1*RrRcFAXXkvJPrV0ySuoguA.png)
...
![](https://miro.medium.com/v2/resize:fit:700/1*usOhxs-zq8yXvGtPwschbQ.png)
...
![](https://miro.medium.com/v2/resize:fit:700/1*nGFYkV2T3f3nmIsArw_0bg.png)
...
It began with [Plugin v1](https://nx.dev/deprecated/v1-nx-plugin-api), which only assigned a list of targets for existing projects (see [🚡 Nx Targets Elevated](https://jgelin.medium.com/nx-targets-elevated-part-one-f421062818e6)).

![](https://miro.medium.com/v2/resize:fit:700/1*qU_0WQZU7QJrjOB93HYwOw.png)

Then came [Plugin v2](https://nx.dev/extending-nx/recipes/project-graph-plugins), offering the ability to dynamically add project nodes with full configuration, even if they do not exist in your codebase (see [✌️ Nx Plugin v2: Dynamic Project Configurations](https://jgelin.medium.com/%EF%B8%8F-nx-plugin-v2-dynamic-project-configurations-6055ba700105)).

![](https://miro.medium.com/v2/resize:fit:700/1*mcfSzTVAJm_3TRIdKxCKCA.png)

And finally, a consolidated approach to that concept was integrated into Nx:

![](https://miro.medium.com/v2/resize:fit:700/0*TImMnQrUaEGs0KTH.jpg)
...
![](https://miro.medium.com/v2/resize:fit:700/1*lTSmh5cI1mt4FLCd0RcBYw.png)

This is true for Vite, but it is also applies to many other plugins, such as:

* [@nx/webpack](https://nx.dev/nx-api/webpack#how-nxwebpack-infers-tasks) → `build`, `preview`, `serve` and `serve-static`
    
* [@nx/cypress](https://nx.dev/nx-api/cypress/documents/overview#how-nxcypress-infers-tasks) → `e2e`, `e2e-ci` and `component-test`
    
* [@nx/playwright](https://nx.dev/nx-api/playwright/documents/overview#how-nxplaywright-infers-tasks) → `e2e`, `e2e-ci`
    
* [@nx](https://nx.dev/nx-api/detox/documents/overview#how-nxdetox-infers-tasks)/[detox](https://nx.dev/nx-api/detox/documents/overview#how-nxdetox-infers-tasks) → `build`, `start` and `test`
    
* [@nx/eslint](https://nx.dev/nx-api/eslint/documents/overview#how-nxeslint-infers-tasks) → `lint`
    
* [@nx/expo](https://nx.dev/nx-api/expo/documents/overview#how-nxexpo-infers-tasks) → `start`, `serve`, `run-ios`, `run-android`, `export`, `prebuild`, `install`, `build` and `submit`
    
* [@nx/jest](https://nx.dev/nx-api/jest/documents/overview#how-nxjest-infers-tasks) → `test`
    
* [@nx/next](https://nx.dev/nx-api/next/documents/overview#how-nxnext-infers-tasks) → `build`, `dev`, `start` and `serve-static`
    
* [@nx/nuxt](https://nx.dev/nx-api/nuxt/documents/overview#how-nxnuxt-infers-tasks) → `build`, `test` and `serve`
    
* [@nx/react-native](https://nx.dev/nx-api/react-native/documents/overview#how-nxreactnative-infers-tasks) → `start`, `pod-install`, `bundle`, `run-ios`, `run-android`, `build-io` and `build-android`
    
* [@nx/remix](https://nx.dev/nx-api/remix/documents/overview#how-nxremix-infers-tasks) → `build`, `serve`, `start` and `typecheck`
    
* [@nx/storybook](https://nx.dev/nx-api/storybook/documents/overview#how-nxstorybook-infers-tasks) → `build-storybook`, `storybook`, `test-storybook` and `static-storybook`
    

**Why don’t I see the main plugins like**[**@nx/angular**](https://nx.dev/nx-api/angular/documents/overview)**or**[**@nx/react**](https://nx.dev/nx-api/react/documents/overview)**?**
...
> *IMO, it’s only a matter of time before the*[Nx](https://medium.com/u/2817fb68583?source=post_page-----7f42faf2a135--------------------------------) team proposes a solution for this :).

# **How it works**

Let’s take a look behind the scenes to understand how we can benefit from this feature.

![](https://miro.medium.com/v2/resize:fit:700/1*ZRLyFoT9uIRc3liNAfQmzQ.png)
...
![](https://miro.medium.com/v2/resize:fit:700/1*pZIigX5sT2oYliA39e8NPA.png)
...
And again, you’ll need to [register your plugin in your `nx.json`](https://nx.dev/plugins/recipes/project-inference-plugins#adding-plugins-to-workspace) configuration file:
...
![](https://miro.medium.com/v2/resize:fit:700/1*OP6MbLdELLjk0J-D7_N8cQ.png)
...
Adhering to the “[**Don’t Repeat Yourself**](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself)” principle becomes straightforward. If your workspace is cluttered with redundant `project.json` configurations, creating plugins to inject common elements significantly eases long-term maintenance.

## **Centralize your workspace conventions**

As I discussed in my previous article [⚡ The Super Power of Conventions with Nx](https://jgelin.medium.com/the-super-power-of-conventions-with-nx-8d418150b679), for teams managing large codebases, enhancing the development experience through established conventions is invaluable. These can be rules or best practices, encapsulated within an Nx plugin to uniformly apply across your workspace.
...
> *If you are interested in technical details, you can also read my article*[*⚡ Distributed e2e Task Execution with Nx for Playwright and Cypress*](https://medium.com/@jgelin/nx-distribute-e2e-task-execution-for-playwright-and-cypress-e3aa8811842a)

## **Less complex generators and migrations**

When I started using the project inference, the main concern was **“It will break the**[**migration process of Nx**](https://nx.dev/extending-nx/recipes/migration-generators)**!”**
...
![](https://miro.medium.com/v2/resize:fit:700/1*K-PsJbGqMYmaXmA1-5WgHA.png)
...
![](https://miro.medium.com/v2/resize:fit:700/1*ulXxZBvERw-o5DN-DVfIRg.png)
