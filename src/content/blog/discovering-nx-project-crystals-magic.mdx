...
It began with [Plugin v1](https://nx.dev/deprecated/v1-nx-plugin-api), which only assigned a list of targets for existing projects (see [🏘️ Poly-Monorepos with Nx](/blog/poly-monorepos-with-nx)).

![](\/blog/images/1*qU_0WQZU7QJrjOB93HYwOw.png)

Then came [Plugin v2](https://nx.dev/extending-nx/recipes/project-graph-plugins), offering the ability to dynamically add project nodes with full configuration, even if they do not exist in your codebase (see [💡 10 Tips for Successful Nx Plugin Architecture](/blog/10-tips-for-successful-nx-plugin-architecture)).
